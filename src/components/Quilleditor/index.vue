<template>
  <div>
    ...
    <CropUpload v-model="showCrop" :width="width" :height="height" :fileName="fileName" :uploadUrl="uploadUrl"
      @uploadSuccess="onUploadSuccess"></CropUpload>
  </div>
</template>
<script>
  import {
  ...
  export default {
    ...
    methods: {
      ...
      /*选择上传图片切换，在这个方法中即进行上传操作，上传后将图片渲染回编辑器*/
      onFileChange(e) {
      ...
      onUploadSuccess: function(response) {
        console.log(response)
        this.editor.focus();
        this.editor.insertEmbed(this.editor.getSelection().index, 'image', response.data.data);
      },
      /*点击上传图片按钮*/
      imgClick() {
        if (this.canCrop) {
